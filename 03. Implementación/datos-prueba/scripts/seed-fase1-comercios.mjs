...
const FOTOS_PRODUCTO_DIR = path.join(DATOS_PRUEBA_DIR, "fotos-prueba");
...
const COMERCIOS = [
    ...
    },
    {
        ...
        productos: [
            ...
            ["Combo 2 Cheese Bacon Doble + Papas + Gaseosa", "Dos Cheese Bacon Doble, papas fritas para compartir y gaseosa en lata.", 34000, "Hamburguesas", ["Combos", "Para Compartir"], "bigburger-combo2cheesebacondoblepapasgaseosa.jpg"]
        ]
    },
    {
        key: "tantesara",
...
async function apiPost(pathSegment, body, token) {
...
async function subirACloudinary(filePath, firma) {
    const buffer = await readFile(filePath);
    const form = new FormData();
    form.append("file", new Blob([buffer]), path.basename(filePath));
    form.append("api_key", firma.data.apiKey);
    form.append("timestamp", String(firma.data.timestamp));
    form.append("signature", firma.data.signature);
    form.append("folder", firma.data.folder);
    form.append("upload_preset", firma.data.uploadPreset);

    const res = await fetch(`https://api.cloudinary.com/v1_1/${firma.data.cloudName}/image/upload`, {
        method: "POST",
        body: form
    });
...
for (const c of COMERCIOS) {
    ...
    try {
        ...
        const fotoPerfilUrl = await subirACloudinary(path.join(DATOS_PRUEBA_DIR, c.fotoPerfil), firmaFoto);
        ...
        console.log(`  Registrado. usuarioId=${registro.usuarioId}`);

        registro.productos = [];
        // El login/estado se activa después vía SQL directo (ver activar-comercios.sql),
        // así que acá cortamos: la carga de productos sigue en seed-fase1-productos.mjs
        // una vez corrida esa activación (login requiere usuario.estado=ACTIVO).
...
await import("node:fs/promises").then(fs => fs.writeFile(outPath, JSON.stringify({ comercios: resumen, COMERCIOS_DATA_KEYS: COMERCIOS.map(c => c.key) }, null, 2)));
console.log(`\nResumen guardado en ${outPath}`);

export { COMERCIOS, catId, tagIds, apiPost, subirACloudinary, FOTOS_PRODUCTO_DIR, DATOS_PRUEBA_DIR };
